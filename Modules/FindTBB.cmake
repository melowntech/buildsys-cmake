...
foreach (_TBB_COMPONENT IN LISTS _TBB_FIND_COMPONENTS)
  ...
  if (TBB_${_TBB_COMPONENT}_LIBRARY_RELEASE AND TBB_${_TBB_COMPONENT}_LIBRARY_DEBUG)
    set(TBB_${_TBB_COMPONENT}_LIBRARY optimized ${TBB_${_TBB_COMPONENT}_LIBRARY_RELEASE} debug
                                      ${TBB_${_TBB_COMPONENT}_LIBRARY_DEBUG})
  ...
  if (TBB_${_TBB_COMPONENT}_FOUND)
    ...
    if (NOT TARGET ${_TBB_TARGET_NAME}) # Allow multiple calls of this script
      ...
      set_target_properties(
        ${_TBB_TARGET_NAME}
        PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${TBB_${_TBB_COMPONENT}_INCLUDE_DIRS}"
                   IMPORTED_LINK_INTERFACE_LANGUAGES CXX
                   IMPORTED_NO_SONAME TRUE)
      if (_TBB_${_TBB_COMPONENT}_LIB_LINK_DEPENDS)
        set_target_properties(
          ${_TBB_TARGET_NAME} PROPERTIES INTERFACE_LINK_LIBRARIES
      ...
      foreach (_TBB_CONFIGURATION IN ITEMS DEBUG RELEASE)
        if (TBB_${_TBB_COMPONENT}_LIBRARY_${_TBB_CONFIGURATION})
          ...
          if (WIN32)
            set_target_properties(
              ${_TBB_TARGET_NAME}
              PROPERTIES IMPORTED_IMPLIB_${_TBB_CONFIGURATION}
                         "${TBB_${_TBB_COMPONENT}_LIBRARY_${_TBB_CONFIGURATION}}")
            string(REPLACE "/lib/" "/bin/" _TBB_LIB_PATH_DLL
                           "${TBB_${_TBB_COMPONENT}_LIBRARY_${_TBB_CONFIGURATION}}")
